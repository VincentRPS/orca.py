...
from orca.types.base import UNSET
from orca.util.config import Config
from orca.util.string import underscore
from orca.util.hashmap import HashMap, DefaultHashMap
...
class State(object):
    ...
    the `GatewayClient`. State tracking is a core component of the orca client,
    ...
    def on_guild_delete(self, event):
        ...
        if event.id in self.voice_clients:
            self.voice_clients[event.id].orcannect()
    ...
    def on_voice_state_update(self, event):
        # Existing connection, we are either moving channels or orcannecting
