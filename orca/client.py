...
from orca.api.client import APIClient
from orca.gateway.client import GatewayClient
from orca.gateway.packets import OPCode
from orca.types.user import Status, Game
from orca.util.config import Config
from orca.util.emitter import Emitter
from orca.util.logging import LoggingClass
from orca.util.backdoor import orcaBackdoorServer
...
class ClientConfig(Config):
    ...
    ----------
    token : str
        orcard authentication token, can be validated using the
        `orca.util.token.is_valid_token` function.
    shard_id : int
...
class Client(LoggingClass):
    ...
    (`orca.api.client.APIClient`) and the realtime gateway API
    (`orca.gateway.client.GatewayClient`).
    ...
    def __init__(self, config):
        ...
        if self.config.manhole_enable:
            ...
            self.manhole = orcaBackdoorServer(
