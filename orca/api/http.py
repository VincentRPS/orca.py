...
from orca import VERSION as orca_version
from requests import __version__ as requests_version
from orca.util.logging import LoggingClass
from orca.api.ratelimit import RateLimiter
...
class HTTPClient(LoggingClass):
    ...
    orcards rate-limit headers, authorization, and request/response validation.
    """
    ...
    def __init__(self, token, after_request=None):
        ...
        self.session.headers.update(
            {
                "User-Agent": "DiscordBot (https://github.com/VincentRPS/orca.py {}) Python/{} requests/{}".format(
                    orca_version, py_version, requests_version
                ),
            }
        )
    ...
    def call(self, route, args=None, **kwargs):
        ...
        :class:`orca.api.http.Routes`) with a set of URL arguments, and keyword
        ...
        filtered = {
            k: (v if k in ("guild", "channel") else "") for k, v in six.iteritems(args)
        }
