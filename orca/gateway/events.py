...
from orca.types.user import User, Presence
from orca.types.channel import Channel, PermissionOverwrite
from orca.types.message import Message, MessageReactionEmoji
from orca.types.voice import VoiceState
from orca.types.guild import Guild, GuildMember, Role, GuildEmoji
...
from orca.util.string import underscore

# Mapping of orcards event name to our event classes
EVENTS_MAP = {}
...
class Ready(GatewayEvent):
    ...
    session_id : str
        The session ID.
    user : :class:`orca.types.user.User`
        The user object for the authenticated account.
    guilds : list[:class:`orca.types.guild.Guild`
        All guilds this account is a member of. These are shallow guild objects.
    private_channels list[:class:`orca.types.channel.Channel`]
...
class GuildCreate(GatewayEvent):
    ...
    Attributes
    ----------
    guild : :class:`orca.types.guild.Guild`
...
class ChannelCreate(GatewayEvent):
    ...
    Attributes
    ----------
    channel : :class:`orca.types.channel.Channel`
...
class GuildEmojisUpdate(GatewayEvent):
    ...
    emojis : list[:class:`orca.types.guild.Emoji`]
...
class GuildMembersChunk(GatewayEvent):
    ...
    members : list[:class:`orca.types.guild.GuildMember`]
        The chunk of members.
    not_found : list[snowflake]
        An array of invalid requested guild members.
    presences : list[:class:`orca.types.user.Presence`]
...
class GuildMemberAdd(GatewayEvent):
    ...
    Attributes
    ----------
    member : :class:`orca.types.guild.GuildMember`
...
class GuildRoleCreate(GatewayEvent):
    ...
    role : :class:`orca.types.guild.Role`
...
class MessageCreate(GatewayEvent):
    ...
    Attributes
    ----------
    message : :class:`orca.types.message.Message`
...
class PresenceUpdate(GatewayEvent):
    ...
    Attributes
    ----------
    presence : :class:`orca.types.user.Presence`
...
class VoiceStateUpdate(GatewayEvent):
    ...
    Attributes
    ----------
    state : :class:`orca.models.voice.VoiceState`
...
class MessageReactionAdd(GatewayEvent):
    ...
    emoji : :class:`orca.types.message.MessageReactionEmoji`
