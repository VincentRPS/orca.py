...
from orca.types.guild import GuildMember
from orca.bot.plugin import find_loadable_plugins
from orca.bot.command import CommandEvent, CommandLevels
from orca.bot.storage import Storage
from orca.util.config import Config
from orca.util.logging import LoggingClass
from orca.util.serializer import Serializer
from orca.util.threadlocal import ThreadLocal
from orca.util.enum import get_enum_value_by_name
...
class BotConfig(Config):
    ...
    levels : dict(snowflake, str)
        Mapping of user IDs/role IDs to :class:`orca.bot.commands.CommandLevels`
    ...
    commands_level_getter : function
        ...
        relevant :class:`orca.bot.commands.CommandLevels`.
...
class Bot(LoggingClass):
    """
    orca's implementation of a simple but extendable orcard bot. Bots consist
    of a set of plugins, and a orca Client.

    Parameters
    ----------
    client : :class:`orca.client.Client`
    ...
    def from_cli(cls, *plugins):
        ...
        :mod:`orca.cli` module. Allows passing in a set of uninitialized
        plugin classes to load.

        Parameters
        ---------
        plugins : Optional[list(:class:`orca.bot.plugin.Plugin`)]
            Any plugins to load after creating the new bot instance.
        """
        from orca.cli import orca_main
    ...
    def get_commands_for_message(self, require_mention, mention_rules, prefixes, msg):
        ...
        msg : :class:`orca.types.message.Message`
            The message object to parse and find matching commands for.

        Yields
        -------
        tuple(:class:`orca.bot.command.Command`, `re.MatchObject`)
    ...
    def add_plugin(self, inst, config=None, ctx=None):
        ...
        Parameters
        ----------
        inst : subclass (or instance therein) of `orca.bot.plugin.Plugin`
    ...
    def rmv_plugin(self, cls):
        ...
        Parameters
        ----------
        cls : subclass of :class:`orca.bot.plugin.Plugin`
