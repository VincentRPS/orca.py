...
from orca.util.functional import one_or_many, chunks
...
from orca.types.permissions import Permissions, Permissible, PermissionValue
...
class PermissionOverwrite(ChannelSubType):
    ...
    id : snowflake
        The overwrite ID.
    type : :const:`orca.types.channel.PermissionsOverwriteType`
        The overwrite type.
    allow : :class:`orca.types.permissions.PermissionValue`
        All allowed permissions.
    deny : :class:`orca.types.permissions.PermissionValue`
    ...
    def create_for_channel(cls, channel, entity, allow=0, deny=0):
        from orca.types.guild import Role
...
class Channel(SlottedModel, Permissible):
    ...
    recipients : list(:class:`orca.types.user.User`)
        Members of this channel (if this is a DM channel).
    type : :const:`ChannelType`
        The type of this channel.
    overwrites : dict(snowflake, :class:`orca.types.channel.PermissionOverwrite`)
    ...
    def after_load(self):
        # TODO: hackfix
        self.attach(
            six.itervalues(self.overwrites), {"channel_id": self.id, "channel": self}
        )
    ...
    def create_invite(self, *args, **kwargs):
        ...
        from orca.types.invite import Invite
...
class MessageIterator(object):
    ...
    client : :class:`orca.client.Client`
        The orca client instance to use when making requests.
    channel : `Channel`
    ...
    def fill(self):
        """
        Fills the internal buffer up with :class:`orca.types.message.Message` objects from the API.
